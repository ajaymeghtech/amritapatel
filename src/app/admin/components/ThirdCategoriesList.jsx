...
export default function ThirdCategoriesList() {
  ...
  const handleFormSubmit = async (e, closeAfterSubmit = false) => {
    ...
    if (!formData.subActivityId) {
      toast.error("Please select a sub-activity");
      return;
    }
    ...
    try {
      ...
      payload.append("subActivityId", formData.subActivityId);
      ...
      let response;
      if (formMode === "edit") {
        response = await fetch(`${THIRD_CATEGORIES_ENDPOINT}/${editId}`, {
          method: "PUT",
          body: payload,
        });
      } else {
      ...
      toast.success(formMode === "edit" ? "Third category updated!" : "Third category created!");
      ...
      if (formMode === "edit" || closeAfterSubmit) {
        ...
        setFormData({
          subActivityId: formData.subActivityId, // Keep sub-activity selected
          title: "",
          short_description: "",
          description: "",
          image: "",
  ...
  const handleDeleteConfirm = async () => {
    try {
      await fetch(`${THIRD_CATEGORIES_ENDPOINT}/${confirmModal.id}`, { method: "DELETE" });
  ...
  // Filter third categories
  const filteredThirdCategories = useMemo(() => {
    return thirdCategories.filter((item) => {
      const subActivityMatch = subActivityFilter === "all" || item.subActivityId === subActivityFilter;
      const searchMatch = searchTerm === "" ||
        item.title?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        item.short_description?.toLowerCase().includes(searchTerm.toLowerCase());
      return subActivityMatch && searchMatch;
    });
  }, [thirdCategories, subActivityFilter, searchTerm]);

  // Get sub-activity name by ID
  const getSubActivityName = (subActivityId) => {
    const subActivity = subActivities.find(sa => sa._id === subActivityId);
    return subActivity?.title || subActivityId || "—";
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 200,
      renderCell: (params) => (
        <Tooltip content={getSubActivityName(params.value)}>
          <div className="fw-medium text-truncate" style={{ maxWidth: '180px' }} title={getSubActivityName(params.value)}>
            {getSubActivityName(params.value)}
          </div>
        </Tooltip>
      ),
    },
  ...
  return (
    <div style={{
        ...
        <div className={styles.actionBarContent}>
          ...
          <div className={styles.actionBarRight}>
            ...
            >
              ...
              <span className={styles.buttonTextFull}>Add Third Category</span>
      ...
      <div style={{ width: "100%", overflowX: "auto" }}>
        <CommonDataGrid
          data={filteredThirdCategories.map((item, index) => ({ ...item, id: item._id, srNo: index + 1 }))}
          columns={columns}
          loading={loading}
          pageSizeOptions={[5, 10, 15, 20, 50]}
          initialPageSize={10}
          noDataMessage="No third categories found"
          noDataDescription={
            searchTerm || subActivityFilter !== "all"
              ? "Try adjusting your search criteria or filters."
              : "Get started by creating your first third category."
          }
          noDataAction={
            (!searchTerm && subActivityFilter === "all") ? {
              onClick: () => setFormMode("add"),
              text: "Create First Third Category"
            } : null
          }
          loadingMessage="Loading third categories..."
      ...
      <Modal
        ...
        onClose={resetForm}
        title={formMode === "edit" ? "Edit Third Category" : "Create Third Category"}
      >
        <div className={styles.modalFormContent}>
          <form onSubmit={(e) => handleFormSubmit(e, false)} className={styles.formContainer}>
            <div className={styles.formSection}>
              <div className={styles.formField}>
                <label className={styles.formLabel}>
                  Sub-Activity <span className={styles.required}>*</span>
                </label>
                <select
            ...
            <div className={styles.formActions}>
              ...
              >
                {formMode === "edit" ? "Update Third Category" : "Create Third Category"}
              </button>
            </div>
          </form>
        </div>
